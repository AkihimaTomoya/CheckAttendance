...
class CustomInputPreprocessor(nn.Module):
    ...
    def forward(self, rgb_images):
        grayscale = TF.rgb_to_grayscale(rgb_images)
        mask = self.mask_extractor.get_face_mask(rgb_images)

        # noise = torch.randn_like(grayscale) * self.noise_std
        # noisy = torch.clamp(grayscale + noise, 0.0, 1.0)

        # return torch.cat([grayscale, mask, noisy], dim=1)
        return torch.cat([grayscale, mask], dim=1)
...
class IResNet(nn.Module):
    ...
    def __init__(self,
                 ...
                 groups=1, width_per_group=64, replace_stride_with_dilation=None, fp16=False, use_preprocessing=True, noise_std=0.0):
